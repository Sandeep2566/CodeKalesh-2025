...
router.get('/predict-price-for-batch/:batchId', async (req, res) => {
  ...
  const batchRes = await axios.get(`${BACKEND_BASE}/api/blockchain/batch/${batchId}`);
    ...
    // Call ML service
  const mlRes = await axios.post(`${ML_BASE}/predict-price`, { features });
...
router.get('/detect-quality-anomaly-for-batch/:batchId', async (req, res) => {
  ...
  const batchRes = await axios.get(`${BACKEND_BASE}/api/blockchain/batch/${batchId}`);
    ...
    const features = [quantity, numTransfers, avgPrice, lastQuality, daysSinceCreation];
    // Call ML service
  const mlRes = await axios.post(`${ML_BASE}/detect-quality-anomaly`, { features });
